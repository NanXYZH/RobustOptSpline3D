...
__constant__ float* gpu_bg_ele[3];
__constant__ float* gpu_bg_ele_value[1];
__constant__ float* gpu_bg_ele_normal[3];
__constant__ float* gpu_bg_ele_hessian[9];
__constant__ float* gpu_bg_ele_normal_direction[1];
...
#else 
	//return 1 / (1 + expf(-(x - 1.5) / para_func)) + 1;  // sigmoid_c --> use for ordspline self-supporting optimization
	return 1 / (1 + expf(-(x - 1.5) / para_func));  // sigmoid_c --> use for robspline self-supporting optimization
#endif
}
...
__device__ float inner_(float x, float para_func) {
#if 1 // 函数值连续， 导数不连续
	return x > 1 ? para_func * (x - 1) + 1 : 1;
#elif 0 // 函数值连续， 导数不连续 (nonono)
	return x > 1 ? para_func * x * x : 1;
#else // 大于1的数值还是很小，但是导数连续
	return x > 1 ? para_func * (x - 1) * (x - 1) + 1 : 1;
#endif	
}

__device__ float dinner_(float x, float para_func) {
#if 1 // 函数值不连续， 导数不连续
	return x > 1 ? para_func : 1;
#elif 0 // 函数值连续，导数不连续 (nonono)
	return x > 1 ? 2 * para_func * x : 0;
#else // 大于1的数值还是很小，但是导数连续
	return x > 1 ? 2 * para_func * (x - 1) : 0;
#endif	
}

__device__ float overhang(float x, float sigmoid_mid, float sigmoid_c) {
	return 2 / (1 + expf(-(x - sigmoid_mid) / sigmoid_c));  // sigmoid_c --> use for robspline self-supporting optimization
}

__device__ float doverhang(float x, float sigmoid_mid, float sigmoid_c) {
	return 2 / (1 + expf(-(x - sigmoid_mid) / sigmoid_c)) / (1 + expf(-(x - sigmoid_mid) / sigmoid_c)) / sigmoid_c * expf(-(x - sigmoid_mid) / sigmoid_c); // sigmoid_c
}

__device__ float normal_distri(float x, float para_func){
	return expf(-para_func * (x - 0.5) * (x - 0.5)); // 0.5 means the average of coeff_min and coeff_max
}

__device__ float dnormal_distri(float x, float para_func) {
	return -2 * para_func * (x - 0.5) * expf(-para_func * (x - 0.5) * (x - 0.5)); // 0.5 means the average of coeff_min and coeff_max
}
...
void grid::Grid::uploadSurfaceSymbol2device(void)
{
...
void grid::Grid::uploadbgSymbol2device(void)
{
	// maybe can delete this part, for the bg_ele does not change during the iteration
	cudaMemcpyToSymbol(gpu_bg_ele, _gbuf.bg_ele, sizeof(gpu_bg_ele));
	cuda_error_check;

	cudaMemcpyToSymbol(gpu_bg_ele_value, &_gbuf.bg_ele_value, sizeof(gpu_bg_ele_value));
	cuda_error_check;

	cudaMemcpyToSymbol(gpu_bg_ele_normal, _gbuf.bg_ele_normal, sizeof(gpu_bg_ele_normal));
	cuda_error_check;

	cudaMemcpyToSymbol(gpu_bg_ele_normal_direction, &_gbuf.bg_ele_normal_direction, sizeof(gpu_bg_ele_normal_direction));
	cuda_error_check;

//#ifdef ENABLE_DRIP
	cudaMemcpyToSymbol(gpu_bg_ele_hessian, _gbuf.bg_ele_hessian, sizeof(gpu_bg_ele_hessian));
	cuda_error_check;
//#endif
}
...
void HierarchyGrid::setPrintAngle(float default_angle_ratio, float opt_angle_ratio)
{
	...
	float sigmoid_mid = cosf(M_PI) / cosf(sdefault);
	std::cout << "--[TEST] adapt_sigmoid_mid: " << sigmoid_mid << std::endl;
	_setting.adapt_sigmoid_mid = sigmoid_mid;
	Grid::adapt_sigmoid_mid = sigmoid_mid;

	float sigmoid_c = (sigmoid_mid - 1) / std::log(1 / 0.004 - 1);
	std::cout << "--[TEST] adapt_sigmoid_c: " << sigmoid_c << std::endl;
	_setting.adapt_sigmoid_c = sigmoid_c;
	Grid::adapt_sigmoid_c = sigmoid_c;
}
...
void grid::Grid::uploadBgEleSymbol(void)
{
	cudaMemcpyToSymbol(gpu_bg_ele, _gbuf.bg_ele, sizeof(gpu_bg_ele));
	cuda_error_check;

#ifdef ENABLE_MATLAB
	float* bgpoints_host[3];
	for (int i = 0; i < 3; i++)
	{
		bgpoints_host[i] = new float[n_elements];
		cudaMemcpy(bgpoints_host[i], _gbuf.bg_ele[i] , sizeof(float) * n_elements, cudaMemcpyDeviceToHost);
		cuda_error_check;
	}

	gpu_manager_t::pass_buf_to_matlab("gpu_bgele_x2", bgpoints_host[0], n_elements);
	gpu_manager_t::pass_buf_to_matlab("gpu_bgele_y2", bgpoints_host[1], n_elements);
	gpu_manager_t::pass_buf_to_matlab("gpu_bgele_z2", bgpoints_host[2], n_elements);

	for (int i = 0; i < 3; i++)
	{
		delete[] bgpoints_host[i];
		bgpoints_host[i] = nullptr;
	}
	memset(bgpoints_host, 0, sizeof(bgpoints_host));
#endif
}
...
__global__ void coeff2density_kernel(int nebitword, float mindensity, gBitSAT<unsigned int> esat, int ereso, float* g_dst, coeffdensity calc_node, const int* eidmap, const int* eflag) {
	...
	for (int j = 0; j < BitCount<unsigned int>::value; j++) {
		if (read_gbit(eword, j)) {
			...
			// check the shell element of rho_e
...
{
	//std::cout << "---- check all the size: " << std::endl;
	//std::cout << "n_gselements ---- " << g.n_gselements << std::endl;
	//std::cout << "n_gsvertices ---- " << g.n_gsvertices << std::endl;
	//std::cout << "n_elements ---- " << g.n_elements << std::endl;
	//std::cout << "n_vertices ---- " << g.n_vertices << std::endl;
	//std::cout << "n_eles() ---- " << g.n_eles() << std::endl;
	//std::cout << "n_nodes() ---- " << g.n_nodes() << std::endl;
	//std::cout << "n_valid_elements() ----  " << g.n_valid_elements() << std::endl;
	//std::cout << "n_valid_nodes() ---- " << g.n_valid_nodes() << std::endl;
	...
	for (int i = 0; i < 3; i++) {
		...
		//	if (std::isnan(hostpos[j])) {
		//		//std::cout << "NaN detected at position " << j << std::endl;
		//		NaN_hostpos++;
		//	}
		//}
		//std::cout << "NaN detected count of hostpos at index " << i << " : " << NaN_hostpos << std::endl;

		for (int j = 0; j < g.n_gselements; j++) {
			p3host[j * 3 + i] = hostpos[j];
			//// check NaN
			//if (std::isnan(p3host[j * 3 + i])) {
			//	NaN_count++;
			//}
		}
	}
	//std::cout << "NaN detected count of p3host : " << NaN_count << std::endl;
}

__global__ void computeElementPos2_kernel(int n_word, int vreso, gBitSAT<unsigned int> elesat, devArray_t<double, 3> orig, double eh, devArray_t<double*, 3> pos, const int* eflag) {
	int tid = blockDim.x * blockIdx.x + threadIdx.x;
	if (tid >= n_word) return;
...
void HierarchyGrid::getElementPos_noShell(Grid& g, std::vector<double>& p3host)
{
	int lay = g._layer;
	auto& esat = elesatlist[lay];
	if (esat.total() != g.n_elements) printf("-- error on get element pos\n");
	...
	computeElementPos2_kernel << <grid_size, block_size >> > (nword, vreso, elesat, orig, eh, p, eflag);
	cudaDeviceSynchronize();
	cuda_error_check;
...
// MARK: causing wrong, this function does not need
...
//template<typename bgpoint>
//__global__ void point_kernel(int nebitword, float mindensity, gBitSAT<unsigned int> esat, int ereso, float* g_dst[3], bgpoint calc_point, const int* eidmap, const int* eflag) {
//	int tid = threadIdx.x + blockIdx.x * blockDim.x;
//	if (tid >= nebitword) return;
//
//	const unsigned int* ebit = esat._bitarray;
...
//			int eid = eidoffset + ewordoffset;
//			calc_point(bid);
//			g_dst[0][eid] = gpu_bg_ele[0][bid];
//			g_dst[1][eid] = gpu_bg_ele[1][bid];
//			g_dst[2][eid] = gpu_bg_ele[2][bid];
//			if (eidmap != nullptr) eid = eidmap[eid];
//
//			if (eflag[eid] & grid::Grid::mask_shellelement)
//			{
...
//void grid::Grid::compute_background_point(void)
//{
//	if (_layer != 0) return;
//
//	std::cout << "---- check all the size: " << std::endl;
//	std::cout << "n_gselements ---- " << n_gselements << std::endl;
//	std::cout << "n_gsvertices ---- " << n_gsvertices << std::endl;
//	std::cout << "n_elements ---- " << n_elements << std::endl;
//	std::cout << "n_vertices ---- " << n_vertices << std::endl;
//	std::cout << "n_eles() ---- " << n_eles() << std::endl;
//	std::cout << "n_nodes() ---- " << n_nodes() << std::endl;
//	std::cout << "n_valid_elements() ----  " << n_valid_elements() << std::endl;
//	std::cout << "n_valid_nodes() ---- " << n_valid_nodes() << std::endl;
//
//	// computation
//	float* cijk_value = _gbuf.coeffs;
//	float* knotx_ = _gbuf.KnotSer[0];
...
//	float min_Density = _min_density;
//
//	auto calc_point = [=] __device__(int node_id) {
//		int xCoordi = node_id % ereso;
//		int yCoordi = (node_id % (ereso * ereso)) / ereso;
//		int zCoordi = node_id / (ereso * ereso);
//		float p[3] = { boxOrigin[0] + xCoordi * eh + 0.5 * eh,boxOrigin[1] + yCoordi * eh + 0.5 * eh, boxOrigin[2] + zCoordi * eh + 0.5 * eh };
//
//		int i;
//
//		for (i = 0; i < 3; i++)	gpu_bg_point[i][node_id] = p[i];
//		return;
//	};
//
//	size_t grid_size, block_size;
...
//	gBitSAT<unsigned int> esat(_gbuf.eActiveBits, _gbuf.eActiveChunkSum);
//
//	point_kernel << <grid_size, block_size >> > (_gbuf.nword_ebits, min_Density, esat, _ereso, _gbuf.bg_point, calc_point, _gbuf.eidmap, _gbuf.eBitflag);
//	cudaDeviceSynchronize();
//	cuda_error_check;
//
//	int n = n_gselements;
...
//		for (int i = 0; i < 3; i++)
//		{
//			bg_node[i] = new float[n];
//			cudaMemcpy(bg_node[i], _gbuf.bg_point[i], sizeof(float) * n, cudaMemcpyDeviceToHost);
//			cuda_error_check;
//		}
//		gpu_manager_t::pass_buf_to_matlab("bg_node_x", bg_node[0], n);
//		gpu_manager_t::pass_buf_to_matlab("bg_node_y", bg_node[1], n);
//		gpu_manager_t::pass_buf_to_matlab("bg_node_z", bg_node[2], n);
//		for (int i = 0; i < 3; i++) {
//			delete bg_node[i];
//			bg_node[i] = nullptr;
//		}
...
void grid::Grid::compute_spline_background_ele_value(void)
{
	float* value_tmp;
	cudaMalloc(&value_tmp, sizeof(float) * n_elements);
	init_array(value_tmp, float{ 0 }, n_elements);
	cuda_error_check;

	auto calc_value = [=] __device__(int node_id) {
		float p[3] = { 0.f };
		float normal[3] = { 0.f };
		float val;
		int i, j, k, ir, it, is, index;
		...
		for (i = 0; i < 3; i++) p[i] = gpu_bg_ele[i][node_id];
			...
			for (ir = i - m_iM; ir < i; ir++)
			{
				for (is = j - m_iM; is < j; is++)
				{
					for (it = k - m_iM; it < k; it++)
					{
						...
						val += gpu_cijk[index] * NX[ir - i + m_iM] * NY[is - j + m_iM] * NZ[it - k + m_iM];					}
				}
			}
		}
		value_tmp[node_id] = val;
		return;
	};

	size_t grid_dim, block_dim;
	int n = spline_bg_ele->size();

	make_kernel_param(&grid_dim, &block_dim, n, 256);
	traverse_noret << <grid_dim, block_dim >> > (n, calc_value);
	cudaDeviceSynchronize();
	cuda_error_check;

	float* spline_bg_node_value;
	spline_bg_node_value = new float[n];
	cudaMemcpy(spline_bg_node_value, value_tmp, sizeof(float)* n, cudaMemcpyDeviceToHost);
	cuda_error_check;

#ifdef ENABLE_MATLAB
	gpu_manager_t::pass_buf_to_matlab("spline_bg_ele_value", spline_bg_node_value, n);
#endif

	init_array(_gbuf.bg_ele_value, float{ 0 }, n_elements);
	cudaMemcpy(_gbuf.bg_ele_value, spline_bg_node_value, n_elements * sizeof(float), cudaMemcpyHostToDevice);
	cuda_error_check;
	cudaMemcpyToSymbol(gpu_bg_ele_value, &_gbuf.bg_ele_value, sizeof(gpu_bg_ele_value));
	cuda_error_check;
	...
	cudaFree(value_tmp);
	value_tmp = nullptr;
	delete spline_bg_node_value;
	spline_bg_node_value = nullptr;

}

void grid::Grid::compute_spline_background_ele_normal(void)
{
	auto calc_normal = [=] __device__(int node_id) {
		float p[3] = { 0.f };
		float normal[3] = { 0.f };
		...
		for (i = 0; i < 3; i++)	gpu_bg_ele_normal[i][node_id] = normal[i];
		return;
	};
...
#ifdef ENABLE_MATLAB
	{
		float* spline_bg_node_normal[3];
		for (int i = 0; i < 3; i++)
		{
			spline_bg_node_normal[i] = new float[n];
			cudaMemcpy(spline_bg_node_normal[i], _gbuf.bg_ele_normal[i], sizeof(float) * n, cudaMemcpyDeviceToHost);
			cuda_error_check;
		}
		gpu_manager_t::pass_buf_to_matlab("spline_bg_ele_normal_x", spline_bg_node_normal[0], n);
		gpu_manager_t::pass_buf_to_matlab("spline_bg_ele_normal_y", spline_bg_node_normal[1], n);
		gpu_manager_t::pass_buf_to_matlab("spline_bg_ele_normal_z", spline_bg_node_normal[2], n);
		for (int i = 0; i < 3; i++) {
			delete spline_bg_node_normal[i];
			spline_bg_node_normal[i] = nullptr;
		}
	}
#endif
}

float grid::Grid::correct_spline_background_ele_normal_direction(float beta)
{
	float count = 1;
	float* direction_tmp;
	cudaMalloc(&direction_tmp, sizeof(float) * n_gselements);
	init_array(direction_tmp, float{ 0 }, n_gselements);
	cuda_error_check;

	float el_ = elementLength() * 1e-2;

	float print_angle = _opt_print_angle;

	auto calc_node = [=] __device__(int node_id) {
		float p[3];
		for (int i = 0; i < 3; i++) 	p[i] = gpu_bg_ele[i][node_id];

		float normal_vector[3];
		for (int i = 0; i < 3; i++)		normal_vector[i] = gpu_bg_ele_normal[i][node_id];

		float s = 0.f, tmp = 0.f;
		float delta = el_, p_delta[3] = { 0.f };
		for (int i = 0; i < 3; i++)
		{
			p_delta[i] = p[i] + delta * normal_vector[i];
		}
		...
		else
		{
			...
			for (ir = i - m_iM; ir < i; ir++)
			{
				for (is = j - m_iM; is < j; is++)
				{
					...
					{
						index = ir + is * gnbasis[0] + it * gnbasis[0] * gnbasis[1];
						val_delta += gpu_cijk[index] * NX_delta[ir - i + m_iM] * NY_delta[is - j + m_iM] * NZ_delta[it - k + m_iM];
					}
				}
			}
		}

		s = Heaviside(val, beta) - Heaviside(val_delta, beta);
		if (s > 0)   // s > 0 --> void
		{
			s = 1;
		}
	...
	float* direction_host = new float[n_elements];
	cudaMemcpy(direction_host, direction_tmp, sizeof(float) * n_elements, cudaMemcpyDeviceToHost);
	cuda_error_check;
	init_array(_gbuf.bg_ele_normal_direction, float{ 0 }, n_elements);

	cudaMemcpy(_gbuf.bg_ele_normal_direction, direction_host, n_elements * sizeof(float), cudaMemcpyHostToDevice);
	cuda_error_check;

#ifdef ENABLE_MATLAB
	gpu_manager_t::pass_buf_to_matlab("spline_bg_ele_normal_direction", direction_host, n_elements);
#endif
	...
	cuda_error_check;
	float* tmp = (float*)getTempBuf(sizeof(float) * n / 100);
	count = parallel_sum(_gbuf.bg_ele_normal_direction, tmp, n);
	cudaDeviceSynchronize();
	cuda_error_check;
	...
	if (count < 0)
	{
		...
		float* normal_dirction = (float*)_gbuf.bg_ele_normal_direction;
		auto calc_normal = [=] __device__(int node_id) {
			float normal[3] = { 0.f };

			for (int i = 0; i < 3; i++)
			{
				normal[i] = gpu_bg_ele_normal[i][node_id];
			}

			for (int i = 0; i < 3; i++)	gpu_bg_ele_normal[i][node_id] = -normal[i];
			return;
		};
		traverse_noret << <grid_dim, block_dim >> > (n, calc_normal);
		cudaDeviceSynchronize();
		...
		gpu_manager_t::pass_buf_to_matlab("corrected_bg_ele_normal_x", spline_bg_node_normal[0], n);
		gpu_manager_t::pass_buf_to_matlab("corrected_bg_ele_normal_y", spline_bg_node_normal[1], n);
		gpu_manager_t::pass_buf_to_matlab("corrected_bg_ele_normal_z", spline_bg_node_normal[2], n);
		for (int i = 0; i < 3; i++)		delete spline_bg_node_normal[i];
#endif     		
	}
	return count;
}

void grid::Grid::compute_background_selfsupp_flag_actual(void)
{
	float default_print_angle = _default_print_angle;
	...
	auto calc_node = [=] __device__(int node_id) {
		...
		for (int i = 0; i < 3; i++)
		{
			p[i] = gpu_bg_ele[i][node_id];
		}

		float normal_vector[3] = { 0.f };
		for (int i = 0; i < 3; i++)
		{
			normal_vector[i] = gpu_bg_ele_normal[i][node_id];
		}
	...
	int n = n_elements;
	make_kernel_param(&grid_dim, &block_dim, n, 256);
	traverse << <grid_dim, block_dim >> > ((float*)_gbuf.bg_ele_flag, n, calc_node);
	cudaDeviceSynchronize();
	cuda_error_check;

#ifdef ENABLE_MATLAB 
	float* host_spline_constrain = new float[n];
	cudaMemcpy(host_spline_constrain, _gbuf.bg_ele_flag, sizeof(float) * n, cudaMemcpyDeviceToHost);
	gpu_manager_t::pass_buf_to_matlab("spline_constrain_actual", host_spline_constrain, n);
	delete host_spline_constrain;
	cuda_error_check;
#endif
...
void grid::Grid::compute_background_selfsupp_flag_virtual(void)
{
	float print_angle = _opt_print_angle;
	...
	int n = n_elements;
	make_kernel_param(&grid_dim, &block_dim, n, 256);
	traverse << <grid_dim, block_dim >> > ((float*)_gbuf.bg_ele_flag_virtual, n, calc_node);
	cudaDeviceSynchronize();
	cuda_error_check;

#ifdef ENABLE_MATLAB 
	float* host_spline_constrain = new float[n];
	cudaMemcpy(host_spline_constrain, _gbuf.bg_ele_flag_virtual, sizeof(float) * n, cudaMemcpyDeviceToHost);
	gpu_manager_t::pass_buf_to_matlab("spline_constrain_virtual", host_spline_constrain, n);
	delete host_spline_constrain;
	cuda_error_check;
#endif
...
float grid::Grid::count_bg_points(void)
{
	auto calc_node = [=] __device__(int node_id) {
		float p[3];
		float s = 0.f;
	...
	int n = n_elements;
	make_kernel_param(&grid_dim, &block_dim, n, 256);
	traverse << <grid_dim, block_dim >> > ((float*)_gbuf.bg_ele_buf, n, calc_node);
	cudaDeviceSynchronize();
	cuda_error_check;

	float* tmp = (float*)grid::Grid::getTempBuf(sizeof(float) * n / 100);
	float count = parallel_sum(_gbuf.bg_ele_buf, tmp, n);
	cudaDeviceSynchronize();
	cuda_error_check;
	return count;
}
...
//  Deal with self-supporting 
void grid::Grid::compute_spline_background_selfsupp_constraint(void)
{
	float print_angle = _opt_print_angle;
	int modeid = _ssmode;

	float func_mid = adapt_sigmoid_mid;
	float func_para = adapt_sigmoid_c;
	float indicator_func_para = normal_distri_a;
	float inner_para = inner_scale_a;
	...
	{
		func_para = adapt_sigmoid_c;
	}

	float* func_value = (float*)_gbuf.bg_ele_value;
	...
	auto calc_node = [=] __device__(int node_id) {
		...
		//float func_val = func_value[node_id];
		float func_val = gpu_bg_ele_value[0][node_id];
		...
		else if (modeid == 4)       // overhang_ss
		{
			if (normal_vector[2] < 0)
			{
				val = normal_distri(func_val, indicator_func_para) * overhang(inner, func_mid, func_para) * inner_(inner, inner_para);
			}
		}
		else if (modeid == 5)       // overhang2_ss
		{
	...
	int n = n_elements;
	make_kernel_param(&grid_dim, &block_dim, n, 256);
	traverse << <grid_dim, block_dim >> > ((float*)_gbuf.bg_ss_ele_value, n, calc_node);
	cudaDeviceSynchronize();
	cuda_error_check;

#ifdef ENABLE_MATLAB
	float* host_spline_constrain = new float[n];
	cudaMemcpy(host_spline_constrain, _gbuf.bg_ss_ele_value, sizeof(float) * n, cudaMemcpyDeviceToHost);
	gpu_manager_t::pass_buf_to_matlab("ss_constraint", host_spline_constrain, n);
	delete host_spline_constrain;
	cuda_error_check;
#endif
...
float grid::Grid::global_bg_selfsupp_constraint(void)
{
	float val = 0.f;

	int modeid = _ssmode;
	float func_para = p_norm;
	//float count = grid::Grid::spline_surface_node->size();
	float count = n_elements;

	if (modeid % 2 == 0)
	{
		count = count_bg_points();
	}

	float* tmp = (float*)grid::Grid::getTempBuf(sizeof(float) * n_valid_elements() / 100);
	float ss_sum = parallel_sum(_gbuf.bg_ss_ele_value, tmp, n_valid_elements());
	cudaDeviceSynchronize();
	cuda_error_check;
...
void grid::Grid::compute_spline_bg_selfsupp_constraint_dcoeff(void)
{
	if (_layer != 0) return;

	float* bg_direction = (float*)_gbuf.bg_ele_normal_direction;

		// MARK[TO CHECK] parallel_sum
	//float* tmp = (float*)getTempBuf(sizeof(float) * grid::Grid::spline_surface_node->size() / 100);
	float* tmp = (float*)getTempBuf(sizeof(float) * n_elements / 100);
	cuda_error_check;
	float count = parallel_sum(_gbuf.bg_ele_normal_direction, tmp, n_elements);
	cudaDeviceSynchronize();
	cuda_error_check;
	...
	std::cout << "--[TEST] mode id: " << modeid << std::endl;
	std::cout << "--[TEST] function mid: " << func_mid << std::endl;
	std::cout << "--[TEST] function para: " << func_para << std::endl;
	...
	auto calc_node_value = [=] __device__(int node_id)
	{
		...
		float norm_dcijk = 0.f;
		for (int i = 0; i < 3; i++) normal_vector[i] = gpu_bg_ele_normal[i][node_id];
		float func_val_tmp = func_value[node_id];
		float direction_tmp = normal_dirction[node_id];
		...
		else
		{
			...
			for (ir = i - m_iM; ir < i; ir++)
			{
				for (is = j - m_iM; is < j; is++)
				{
					for (it = k - m_iM; it < k; it++)
					{
						// indicator function term
						float dfunc_val_tmp = NX[ir - i + m_iM] * NY[is - j + m_iM] * NZ[it - k + m_iM];
						...
						else if (modeid == 4)       // overhang_ss
						{
							if (normal_vector[2] < 0)
							{
								val = dnormal_distri(func_val_tmp, indicator_func_para) * dfunc_val_tmp * overhang(inner, func_mid, func_para) * inner_(inner, inner_para) + normal_distri(func_val_tmp, indicator_func_para) * doverhang(inner, func_mid, func_para) * dinner * inner_(inner, inner_para) + normal_distri(func_val_tmp, indicator_func_para) * overhang(inner, func_mid, func_para) * dinner * dinner_(inner, inner_para);
							}
						}
						else if (modeid == 5)       // overhang2_ss
						{
	...
	size_t grid_dim, block_dim;
	int n = grid::Grid::n_valid_elements();
	make_kernel_param(&grid_dim, &block_dim, n, 256);
	traverse_noret << <grid_dim, block_dim >> > (n, calc_node_value);
	cudaDeviceSynchronize();
	cuda_error_check;
...
//  Deal with drip
void grid::Grid::compute_spline_background_drip_constraint(void) {
	float print_angle = _opt_print_angle;
	float angle_epsilon = drip_angle;
	...
	auto calc_node = [=] __device__(int node_id) {
		...
		for (i = 0; i < 3; i++) {
			...
			{
				int tmp = 3 * i + j;
				gpu_bg_ele_hessian[tmp][node_id] = Hessian[i][j];
			}
		}
		...
		if (modeid == 6)       // exp_drip
		{
			if (normal_vector[2] < 0)
			{
				...
				if (func_drip_version == 1)
				{
					val = g_ * tilde_q1 * tilde_q2 * normal_distri(func_val, indicator_func_para);
				}
				else
				{
					val = h_ * tilde_q1 * tilde_q2 * normal_distri(func_val, indicator_func_para);
				}
			}
		}
		else if (modeid == 7)       // exp2_drip
	...
	int n = n_elements;
	make_kernel_param(&grid_dim, &block_dim, n, 256);
	traverse << <grid_dim, block_dim >> > ((float*)_gbuf.bg_drip_ele_value, n, calc_node);
	cudaDeviceSynchronize();
	cuda_error_check;

#ifdef ENABLE_MATLAB
	float* host_spline_constrain = new float[n];
	cudaMemcpy(host_spline_constrain, _gbuf.bg_drip_ele_value, sizeof(float) * n, cudaMemcpyDeviceToHost);
	gpu_manager_t::pass_buf_to_matlab("drip_constraint", host_spline_constrain, n);
	delete host_spline_constrain;
	cuda_error_check;
#endif
...
#ifdef ENABLE_MATLAB
	{
		...
		{
			spline_surf_node_hessian[i] = new float[n];
			cudaMemcpy(spline_surf_node_hessian[i], _gbuf.bg_ele_hessian[i], sizeof(float) * n, cudaMemcpyDeviceToHost);
			cuda_error_check;
		}
		gpu_manager_t::pass_buf_to_matlab("spline_bg_node_hessian_xx", spline_surf_node_hessian[0], n);
		gpu_manager_t::pass_buf_to_matlab("spline_bg_node_hessian_yy", spline_surf_node_hessian[4], n);
		gpu_manager_t::pass_buf_to_matlab("spline_BG_node_hessian_xy", spline_surf_node_hessian[1], n);
		for (int i = 0; i < 9; i++) {
			delete spline_surf_node_hessian[i];
			spline_surf_node_hessian[i] = nullptr;
		}
...
float grid::Grid::global_bg_drip_constraint(void) {
	float val = 0.f;
	...
	float* tmp = (float*)grid::Grid::getTempBuf(sizeof(float) * n_valid_elements() / 100);
	float drip_sum = parallel_sum(_gbuf.bg_drip_ele_value, tmp, n_valid_elements());
	cudaDeviceSynchronize();
	cuda_error_check;

	std::cout << "--[TEST] total  of drip constraints : " << drip_sum << std::endl;
	std::cout << "--[TEST] number of drip constraints : " << count << std::endl;
...
void grid::Grid::compute_spline_bg_drip_constraint_dcoeff(void) {
	if (_layer != 0) return;
	...
	auto calc_node_value = [=] __device__(int node_id)
	{
		...
		float func_val_tmp = func_value[node_id];
		float direction_tmp = normal_dirction[node_id];
		for (int i = 0; i < 9; i++) hessian_vector[i] = gpu_bg_ele_hessian[i][node_id];
					...
					for (it = k - m_iM; it < k; it++)
					{
						...
						if (modeid == 6)       // overhang_drip
						{
							if (normal_vector[2] < 0)
							{
								if (func_drip_version == 1)
								{
									val = dg_ * hdinner * tilde_q1 * tilde_q2 * normal_distri(func_val_tmp, indicator_func_para)
										+ g_ * dtilde_q1 * dinner1 * tilde_q2 * normal_distri(func_val_tmp, indicator_func_para)
										+ g_ * tilde_q1 * dtilde_q2 * dinner2 * normal_distri(func_val_tmp, indicator_func_para)
										+ g_ * tilde_q1 * tilde_q2 * dnormal_distri(func_val_tmp, indicator_func_para);
								}
								else
								{
									val = dh_ * hdinner * tilde_q1 * tilde_q2 * normal_distri(func_val_tmp, indicator_func_para)
										+ h_ * dtilde_q1 * dinner1 * tilde_q2 * normal_distri(func_val_tmp, indicator_func_para)
										+ h_ * tilde_q1 * dtilde_q2 * dinner2 * normal_distri(func_val_tmp, indicator_func_para)
										* h_ * tilde_q1 * tilde_q2 * dnormal_distri(func_val_tmp, indicator_func_para);
								}
							}
						}
						else if (modeid == 7)       // overhang2_drip
