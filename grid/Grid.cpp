...
void HierarchyGrid::writeElementPos(const std::string& filename, Grid& g)
{
	...
	std::vector<float> spline_bg_gsele[3];

	for (int i = 0; i < 3; i++)
	{
		spline_bg_gsele[i].resize(g.n_gselements);
		_gridlayer[0]->spline_bg_ele[i].resize(g.n_elements);
		for (int j = 0; j < g.n_gselements; j++)
		{
			spline_bg_gsele[i][j] = p3host[3 * j + i];
		}
	}

	Eigen::Matrix<double, -1, -1> p3host_;
	p3host_.resize(g.n_elements, 3);
	for (int i = 0; i < 3; i++)
	{
		int non_nan_count = 0;
		for (int j = 0; j < g.n_gselements; j++)
		{
			if (!std::isnan(spline_bg_gsele[i][j]) && non_nan_count < g.n_elements)
			{
				p3host_(non_nan_count, i) = spline_bg_gsele[i][j];
				_gridlayer[0]->spline_bg_ele[i][non_nan_count] = spline_bg_gsele[i][j];
				non_nan_count++;
			}
		}
	}
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < g.n_gselements; j++)
		{
			p3host_(j, i) = spline_bg_gsele[i][j];
		}
	}
	eigen2ConnectedMatlab("gselepos", p3host_);
#endif // ENABLE_MATLAB
}
...
float grid::Grid::adapt_sigmoid_mid;
float grid::Grid::adapt_sigmoid_c;
...
size_t grid::Grid::build(
	...
	if (layer == 0) {
		...
		/* for new self-supp constraint */
		for (int i = 0; i < 3; i++)
		{
			_gbuf.bg_ele[i] = (float*)gm.add_buf(_name + "bgEle" + std::to_string(i), sizeof(float) * ne);
			gbuf_size += sizeof(float) * ne;
			_gbuf.bg_ele_normal[i] = (float*)gm.add_buf(_name + "bgEleNormal" + std::to_string(i), sizeof(float) * ne);
			gbuf_size += sizeof(float) * ne;
		}
		for (int i = 0; i < 9; i++)
		{
			_gbuf.bg_ele_hessian[i] = (float*)gm.add_buf(_name + "bgEleHessian " + std::to_string(i), sizeof(float) * ne);
			gbuf_size += sizeof(float) * ne;
		}
		_gbuf.bg_ele_value = (float*)gm.add_buf(_name + "bgEleValue", sizeof(float) * ne); gbuf_size += sizeof(float) * ne;
		_gbuf.bg_ele_normal_direction = (float*)gm.add_buf(_name + "bgEleNormaldirection", sizeof(float) * ne); gbuf_size += sizeof(float) * ne;
		_gbuf.bg_ss_ele_value = (float*)gm.add_buf(_name + "bg_ss_ele_value", sizeof(float) * ne); gbuf_size += sizeof(float) * ne;
		_gbuf.bg_drip_ele_value = (float*)gm.add_buf(_name + "bg_drip_ele_value", sizeof(float) * ne); gbuf_size += sizeof(float) * ne;
		_gbuf.bg_ele_flag = (float*)gm.add_buf(_name + "bg_ele_flag", sizeof(float) * ne); gbuf_size += sizeof(float) * ne;
		_gbuf.bg_ele_flag_virtual = (float*)gm.add_buf(_name + "bg_ele_flag_virtual", sizeof(float) * ne); gbuf_size += sizeof(float) * ne;
		_gbuf.bg_ele_buf = (float*)gm.add_buf(_name + "bg_ele_buf ", sizeof(float) * ne); gbuf_size += sizeof(float) * ne;
...
void grid::Grid::uploadBgEle(void)
{
	// upload to device
	for (int i = 0; i < 3; i++)
	{
		gpu_manager_t::upload_buf(_gbuf.bg_ele[i], spline_bg_ele[i].data(), sizeof(float) * n_elements);
	}

#if 1
#ifdef ENABLE_MATLAB
	std::vector<float> bg_points_host[3];
	for (int i = 0; i < 3; i++)
	{
		bg_points_host[i].resize(n_elements);
		gpu_manager_t::download_buf(bg_points_host[i].data(), _gbuf.bg_ele[i], sizeof(float) * n_elements);
	}
	Eigen::VectorXf bg_points_x(n_elements);
	Eigen::VectorXf bg_points_y(n_elements);
	Eigen::VectorXf bg_points_z(n_elements);
	std::copy(bg_points_host[0].begin(), bg_points_host[0].end(), bg_points_x.data());
	std::copy(bg_points_host[1].begin(), bg_points_host[1].end(), bg_points_y.data());
	std::copy(bg_points_host[2].begin(), bg_points_host[2].end(), bg_points_z.data());
	eigen2ConnectedMatlab("gpu_bgele_x1", bg_points_x);
	eigen2ConnectedMatlab("gpu_bgele_y1", bg_points_y);
	eigen2ConnectedMatlab("gpu_bgele_z1", bg_points_z);
#endif
#endif
}
...
#pragma omp parallel for
	for (int i = 0; i < reduced_surf_nodes->size(); i++)
	{
		...
		Ray ray1(near_closest, current_point);  // point out of the model (means direction)
...
void grid::Grid::scale_spline_bg_selfsupp_constraint_dcoeff(void)
{
	// save the original output form
	...
	float count = spline_bg_ele->size();
	int modeid = _ssmode;
	if (modeid % 2 == 0) // only nz < 0
	{
	...
	std::cout << " Spline bg elements in constraint: " << count << " / " << n_elements << std::endl;

	// recover the original output form
...
void grid::Grid::scale_spline_bg_drip_constraint_dcoeff(void)
{
	// save the original output form
	std::ios::fmtflags original_flags = std::cout.flags();
	std::streamsize original_precision = std::cout.precision();
