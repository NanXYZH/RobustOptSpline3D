...
namespace grid {
	...
	public:
		...
		static float adapt_sigmoid_mid;
		static float adapt_sigmoid_c;
	...
	public:
		...
		struct {
			...
			float* bg_ele[3];
			float* bg_ele_value;
			float* bg_ele_normal[3];
			float* bg_ele_hessian[9];
			float* bg_ele_normal_direction;

			float* bg_ele_buf;
			float* bg_ss_ele_value;
			float* bg_drip_ele_value;
			float* bg_ele_flag;                  // compute the support constraint: 1 (not support) 0 (support)
			float* bg_ele_flag_virtual;
		...
		std::vector<float> spline_bg_node[3];
		std::vector<float> spline_bg_ele[3];
		...
		float hfunction_c = 10.0f;    // the parameter of h function which change the local constraint to global

		float normal_distri_a = 200.0f;
		float inner_scale_a = 15.0f;
		...
		// about spline for surface points		
		void uploadSurfacePoints(void);
		
		void uploadSurfacePointsSymbol(void);

		void uploadSurfaceSymbol2device(void);

		// about spline for background points
		void uploadBgEle(void);

		void uploadBgEleSymbol(void);

		void uploadbgSymbol2device(void);
		...
		//[mark] constraint for background point
		//void compute_background_point(void);
		void compute_spline_background_ele_value(void);
		void compute_spline_background_ele_normal(void);
		float correct_spline_background_ele_normal_direction(float beta);
		void compute_background_selfsupp_flag_actual(void);
		void compute_background_selfsupp_flag_virtual(void);

		float count_bg_points(void);
		void compute_spline_background_selfsupp_constraint(void);
		float global_bg_selfsupp_constraint(void);
		void compute_spline_bg_selfsupp_constraint_dcoeff(void);
		void scale_spline_bg_selfsupp_constraint_dcoeff(void);

		void compute_spline_background_drip_constraint(void);
		float global_bg_drip_constraint(void);
		void compute_spline_bg_drip_constraint_dcoeff(void);
		void scale_spline_bg_drip_constraint_dcoeff(void);
		
		// may gather them -- > not use
	...
	private:
		...
		struct HierarchySetting {
			...
			float adapt_sigmoid_mid = 1.5;
			float adapt_sigmoid_c = 0.09;
		}_setting;
	...
	public:
		...
		void getElementPos_noShell(Grid& g, std::vector<double>& p3host);
