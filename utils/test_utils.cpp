...
void TestSuit::testMain(const std::string& testname)
{
	...
	if (testname == "surfelements") {
		...
		testOrdinarySplineTopoptMMA();                             // spline + oldss(paper2) + certain
	}
	else if (testname == "testordsplinetopmma_ss3") {
		testOrdinarySplineTopoptMMA_ss3();                         // spline + newss(paper3) + certain
	}
	else if (testname == "testrobtopmma") {
		testRobustMMA();
	}
	else if (testname == "testrobsplinetopmma") {
		testRobustSplineMMA();                                     // spline + oldss(paper2) + uncertain
	}
...
// spline + newss(paper3) + certain
void TestSuit::testOrdinarySplineTopoptMMA_ss3(void)
{
	//grids.lambdatest();
	...
	while (itn++ < 100) {
		printf("\n* \033[32mITER %d \033[0m*\n", itn);
		printf("\n* \033[32mordsplinetopmma_3ss \033[0m*\n");
		Vgoal *= (1 - params.volume_decrease);
		Vc = Vgoal - params.volume_ratio;
		if (Vgoal < params.volume_ratio) Vgoal = params.volume_ratio;
...
void TestSuit::testRobustMMA(void)
{
	...
	//  Initialization of MMA
	MMA::mma_t mma(grids[0]->n_gselements, n_constraint);
	// here set the lower and upper bound of design variables
	mma.init(params.min_rho, 1);
	float volScale = 1e3;
	float sensScale = 1e3;
	...
	while (itn++ < 100) {
		...
		// update density
		scaleVector(grids[0]->getSens(), grids[0]->n_rho(), sensScale);
		gpu_manager_t::pass_dev_buf_to_matlab("volsensbefore", grids[0]->getVolSens(), grids[0]->n_rho());
		scaleVector(grids[0]->getVolSens(), grids[0]->n_rho(), volScale);
